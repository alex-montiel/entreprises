...
function nbre_tache(day, data)
{
	dataSource = $("#data_source").val();
	$.ajax({ 
			type: "POST",
			url: "nbre_tache.php",
			data: "day="+day+"&data="+data+"&id="+dataSource,
			async: false,
			success:function(data)		
			{ 
				if(data == 0 || data == 1)
				{
					$("#nbre_tache").val(data+" tache");
				}
				else
				{
					$("#nbre_tache").val(data+" taches");
				}
			}	
		});
	
	response = $("#nbre_tache").val();
	return response;
}
...
(function($) {
    $.widget("ui.weekCalendar", {
        ...
        refresh : function() {
            ...
            // Tâches
            this.element.find(".day-column-header > a").remove();
            this._updateDayColumnHeader(this.element.find(this.element.find(".day-column-header")));
        ...
        _resizeCalendar : function () {
            ...
            if(options && $.isFunction(options.height)) {
                ...
                this.element.find(".calendar-scrollable-grid").height((calendarHeight - navHeight - headerHeight) - 100);
            }
        },
        ...
        _loadCalEvents : function(dateWithinWeek) {
            ...
            var self = this;
            var options = this.options;
            var id_utilisateur = $("#data_source").val();
            
            date = dateWithinWeek || options.date;
            weekStartDate = self._dateFirstDayOfWeek(date);
            weekEndDate = self._dateLastMilliOfWeek(date);
            
            // Permet de réutiliser la popup de tâches
            ...
            if (typeof options.data == 'string') {
                ...
                $.getJSON(options.data+"?id_utilisateur="+id_utilisateur, jsonOptions, function(data) {
        ...
        _updateDayColumnHeader : function ($weekDayColumns) {
            ...
            self.element.find(".week-calendar-header td.day-column-header").each(function(i, val) {
                	...
                	var day_ajax = self._formatDate(currentDay, 'Y-m-d');
 
                	var today_day = new Date();
                	today_day.setHours(00);
                	today_day.setMinutes(00);
                	today_day.setSeconds(00);
                	today_day.setMilliseconds(00);
                	
            
            
                	
                	if(currentDay < today_day)
                	{
                		$(this).html(dayName + "<br/>" + self._formatDate(currentDay, options.dateFormat));
                	}
                	else if(currentDay.getMonth() == today_day.getMonth() && currentDay.getDate() == today_day.getDate())
                	{
                		 $(this).html(dayName + "<br/>" + self._formatDate(currentDay, options.dateFormat) + "<br/>" + "<a rel=\"shadowbox;height=800px;width=1024px;\" href=\"../tache/affiche_tache.php?day="+day_ajax+"&idUtil="+id_utilisateur+"\" title=\"Taches du jour \" class='popup'>"+nbre_tache(day_ajax, "today"))+"</a>";
                	}
                	else
                	{
                		$(this).html(dayName + "<br/>" + self._formatDate(currentDay, options.dateFormat) + "<br/>" + "<a rel=\"shadowbox;height=800px;width=1024px;\" href=\"../tache/affiche_tache.php?day="+day_ajax+"&idUtil="+id_utilisateur+"\" title=\"Taches du "+self._formatDate(currentDay, 'l d F Y')+"\" class='popup'>"+nbre_tache(day_ajax, ""))+"</a>";
                	}
        ...
        _renderEvent: function (calEvent, $weekDay) {
            ...
            eventHtml = "<div class=\"" + eventClass + " ui-corner-all\">\
                <div class=\"title ui-corner-top\"></div>\
        ...
        _refreshEventDetails : function(calEvent, $calEvent) {
            ...
            $calEvent.find(".comment").text(calEvent.body);
    ...
    $.extend($.ui.weekCalendar, {
        ...
        defaults: {
            ...
            longMonths : ['Janvier','Fevrier','Mars','Avril','Mai','Juin','Juillet','Ao\373t','Septembre','Octobre','Novembre','D\351cembre'],
